...
// Initialize payment and create widget directly
const initializeAndShowWidget = async () => {
  try {
    ...
    // Show widget and create it immediately
    showWidget.value = true;
    await nextTick();
    await createWompiWidget();
    
    console.log('✅ Wompi widget initialized directly:', {
      reference: paymentReference.value,
      amount: props.amount,
      currency: props.currency
    });
...
const createWompiWidget = async () => {
  ...
  try {
    ...
    // For mobile devices, use direct checkout
    if (isMobile()) {
      console.log('📱 Mobile device detected, redirecting to checkout');
      ...
      console.log('💻 Desktop device detected, creating embedded widget');
...
// Handle payment button click - directly initialize and show
...
<template>
  <div class="wompi-direct-widget">
    ...
    <!-- Payment Button - Only show if widget is not displayed -->
    <button 
      v-if="!showWidget"
    ...
    >
      <div class="flex items-center justify-center relative z-10">
        ...
        <span class="font-bold text-lg">
          {{ isLoading ? 'Preparando pago...' : buttonText }}
        </span>
    ...
    <!-- Wompi Widget Container - Shows immediately after click -->
    <div v-if="showWidget" class="mt-6 p-6 border border-gray-200 rounded-xl bg-white shadow-xl widget-container">
      <!-- Header -->
